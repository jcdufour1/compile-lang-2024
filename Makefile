...
C_WARNINGS = -Werror=incompatible-pointer-types \
			 ...
			 -Wno-unused-parameter -Wno-missing-braces -Wno-type-limits -Wno-unused-value -Wno-format-zero-length -Wno-unused-function -Wno-address
...
C_FLAGS_AUTO_GEN= ${C_WARNINGS} \
			     -std=c11 -pedantic -g -I ./third_party/ -I src/util/ -I src/util/auto_gen/ \
			     -D MIN_LOG_LEVEL=${LOG_LEVEL} \
...
${BUILD_DIR}/auto_gen: src/util/auto_gen/auto_gen.c ${DEP_UTIL}
	${CC_COMPILER} ${C_FLAGS_AUTO_GEN} -D IN_AUTOGEN -o ${BUILD_DIR}/auto_gen src/util/params_log_level.c src/util/arena.c src/util/auto_gen/auto_gen.c src/util/local_string.c
