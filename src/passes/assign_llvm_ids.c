...
void assign_llvm_ids_expr(Env* env) {
    ...
    switch (curr_node->type) {
        ...
        case NODE_E_MEMBER_SYM_TYPED:
            node_unwrap_e_member_sym_typed(curr_node)->llvm_id = llvm_id_for_next_var;
        ...
        case NODE_E_FUNCTION_CALL:
            node_unwrap_e_function_call(curr_node)->llvm_id = llvm_id_for_next_var;
            llvm_id_for_next_var += 2;
            return;
        case NODE_E_OPERATOR: {
            Node_e_operator* operator = node_unwrap_e_operator(curr_node);
