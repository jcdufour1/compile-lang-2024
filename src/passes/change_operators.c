...
static Node_operator* change_op_unary(Env* env, Node_unary* unary) {
    switch (unary->token_type) {
        case TOKEN_NOT: {
            Node_expr* new_lhs = change_op_expr(env, unary->child);
            Node_binary* new_bin = node_binary_new(
                node_get_pos_unary(unary),
                new_lhs,
                node_wrap_literal(util_literal_new_from_int64_t(
                    0,
                    TOKEN_INT_LITERAL,
                    node_get_pos_unary(unary)
                )),
                TOKEN_DOUBLE_EQUAL,
                node_get_lang_type_expr(new_lhs)
            );
...
static Node_block* change_op_block(Env* env, Node_block* root) {
    Node_block* new_block = node_block_new(
        root->pos,
        root->is_variadic,
        (Node_vec) {0},
