...
static Tast_stmt* rm_tuple_expr_not_in_assignment(Env* env, Tast_expr* expr);
...
static Tast_stmt* rm_tuple_assignment_tuple(Env* env, Tast_assignment* assign) {
    ...
    unwrap(symbol_lookup(&struct_def_, env, lang_type_get_str(rm_tuple_lang_type(env, tast_expr_get_lang_type(src), tast_expr_get_pos(src)))));
    ...
    for (size_t idx = 0; idx < dest->members.info.count; idx++) {
        unwrap(idx < tast_struct_def_unwrap(struct_def_)->base.members.info.count);
        ...
        vec_append(&a_main, &new_children, tast_expr_wrap(tast_assignment_wrap(curr_assign)));
    }
...
static Tast_function_def* rm_tuple_function_def_new(Env* env, Tast_function_decl* decl) {
    ...
    Tast_def* struct_def_ = NULL;
    unwrap(symbol_lookup(&struct_def_, env, lang_type_get_str(decl->return_type->lang_type)));
...
static Tast_expr* rm_tuple_struct_literal_rhs(
    ...
    Struct_def_base base = {0};
    Tast_def* struct_def_ = NULL;
    unwrap(symbol_lookup(&struct_def_, env, lang_type_get_str(rhs->lang_type)));
    base = tast_struct_def_unwrap(struct_def_)->base;
    ...
    //unwrap(symbol_lookup(&struct_def_, env, vec_at(&fun_decl->return_type->lang_type, 0).str));
    ...
    switch (struct_def_->type) {
        case TAST_STRUCT_DEF: {
            for (size_t idx = 0; idx < members.info.count; idx++) {
                unwrap(idx < struct_def_memb.info.count);
                ...
                unwrap(sym_tbl_add(&vec_at(&env->ancesters, 0)->symbol_table, tast_variable_def_wrap(new_param)));
...
static Tast_expr* rm_tuple_sum_lit_rhs(
    ...
    switch (struct_def_->type) {
        case TAST_STRUCT_DEF: {
            ...
            unwrap(sym_tbl_add(&vec_at(&env->ancesters, 0)->symbol_table, tast_variable_def_wrap(new_tag)));
            ...
            unwrap(sym_tbl_add(&vec_at(&env->ancesters, 0)->symbol_table, tast_variable_def_wrap(new_item)));
...
static Tast_return* rm_tuple_return(Env* env, Tast_return* rtn) {
    ...
    Tast_def* def = NULL;
    unwrap(symbol_lookup(&def, env, env->name_parent_function));
...
static Tast_expr* rm_tuple_raw_union_lit_rhs(Env* env, Tast_raw_union_lit* rhs, Pos assign_pos) {
    ...
    switch (struct_def_->type) {
        case TAST_RAW_UNION_DEF: {
            ...
            Tast_variable_def* new_tag = tast_variable_def_new(
                assign_pos,
                vec_at(&tast_raw_union_def_unwrap(struct_def_)->base.members, (size_t)rhs->tag->data)->lang_type,
                false,
                util_literal_name_new_prefix("tuple_function_param_item")
            );
            unwrap(sym_tbl_add(&vec_at(&env->ancesters, 0)->symbol_table, tast_variable_def_wrap(new_tag)));
...
static Tast_member_access* rm_tuple_sum_access_rhs(Env* env, Tast_sum_access* rhs, Pos assign_pos) {
    ...
    unwrap(symbol_lookup(&struct_def, env, lang_type_get_str(tast_expr_get_lang_type(new_union_callee))));
    ...
    unwrap(symbol_lookup(&union_def, env, lang_type_get_str(vec_at(&tast_struct_def_unwrap(struct_def)->base.members, 1)->lang_type)));
    Str_view union_memb_name = vec_at(&tast_raw_union_def_unwrap(union_def)->base.members, (size_t)rhs->tag->data)->name;
...
static Tast_binary* rm_tuple_binary_not_in_assignment(Env* env, Tast_binary* oper) {
    oper->lhs = tast_expr_unwrap(rm_tuple_expr_not_in_assignment(env, oper->lhs));
    oper->rhs = tast_expr_unwrap(rm_tuple_expr_not_in_assignment(env, oper->rhs));
    return oper;
}

static Tast_unary* rm_tuple_unary_not_in_assignment(Env* env, Tast_unary* unary) {
    unary->child = tast_expr_unwrap(rm_tuple_expr_not_in_assignment(env, unary->child));
    return unary;
}
...
static Tast_index* rm_tuple_index_not_in_assignment(Env* env, Tast_index* index) {
    index->index = tast_expr_unwrap(rm_tuple_expr_not_in_assignment(env, index->index));
    index->callee = tast_expr_unwrap(rm_tuple_expr_not_in_assignment(env, index->callee));
    return index;
}

static Tast_member_access* rm_tuple_member_access_not_in_assignment(Env* env, Tast_member_access* access) {
    access->lang_type = rm_tuple_lang_type(env, access->lang_type, access->pos);
    access->callee = tast_expr_unwrap(rm_tuple_expr_not_in_assignment(env, access->callee));
    return access;
}
...
static Tast_stmt* rm_tuple_expr_not_in_assignment(Env* env, Tast_expr* expr) {
    switch (expr->type) {
        case TAST_OPERATOR:
            return tast_expr_wrap(tast_operator_wrap(rm_tuple_operator_not_in_assignment(env, tast_operator_unwrap(expr))));
        case TAST_SYMBOL:
            return tast_expr_wrap(rm_tuple_symbol_not_in_assignment(env, tast_symbol_unwrap(expr)));
        case TAST_MEMBER_ACCESS:
            return tast_expr_wrap(tast_member_access_wrap(rm_tuple_member_access_not_in_assignment(env, tast_member_access_unwrap(expr))));
        case TAST_INDEX:
            return tast_expr_wrap(tast_index_wrap(rm_tuple_index_not_in_assignment(env, tast_index_unwrap(expr))));
        case TAST_LITERAL:
            return tast_expr_wrap(expr);
        case TAST_STRUCT_LITERAL:
            unreachable("");
        case TAST_FUNCTION_CALL:
            return tast_expr_wrap(tast_function_call_wrap(rm_tuple_function_call(env, tast_function_call_unwrap(expr))));
        case TAST_TUPLE:
            unreachable("");
        case TAST_SUM_CALLEE:
            unreachable("");
        case TAST_SUM_CASE:
            return tast_expr_wrap(tast_literal_wrap(tast_enum_lit_wrap(tast_sum_case_unwrap(expr)->tag)));
        case TAST_SUM_ACCESS:
            todo();
        case TAST_ASSIGNMENT:
            return rm_tuple_assignment(env, tast_assignment_unwrap(expr));
    }
    unreachable("");
}
...
static Tast_stmt* rm_tuple_stmt(Env* env, Tast_stmt* stmt) {
    switch (stmt->type) {
        ...
        case TAST_EXPR:
            return rm_tuple_expr_not_in_assignment(env, tast_expr_unwrap(stmt));
        case TAST_FOR_RANGE:
