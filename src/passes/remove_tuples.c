...
static Tast_symbol_typed* rm_tuple_symbol_typed_new_from_var_def(const Tast_variable_def* var_def) {
    return tast_symbol_typed_new(var_def->pos, (Sym_typed_base) {
    ...
    switch (lang_type.type) {
        ...
        case LANG_TYPE_TUPLE: {
            ...
            for (size_t idx = 0; idx < lang_type_unwrap_tuple_const(lang_type).lang_types.info.count; idx++) {
                Tast_variable_def* memb = tast_variable_def_new(
                    lang_type_pos,
                    vec_at_const(lang_type_unwrap_tuple_const(lang_type).lang_types, idx),
                    false,
                    util_literal_name_new_prefix("tuple_struct_member")
                );
                vec_append(&a_main, &members, memb);
...
static Tast_stmt* rm_tuple_assignment_tuple(Env* env, Tast_assignment* assign) {
    ...
    Lang_type new_var_lang_type = lang_type_wrap_struct_const(
        lang_type_struct_new(lang_type_atom_new(tast_unwrap_struct_def(struct_def_)->base.name, 0))
    );
    ...
    for (size_t idx = 0; idx < dest->members.info.count; idx++) {
        ...
        Tast_member_access_typed* curr_src = tast_member_access_typed_new(
            tast_get_pos_expr(src),
            curr_memb_def->lang_type,
            curr_memb_def->name,
            tast_wrap_symbol_typed(rm_tuple_symbol_typed_new_from_var_def(new_var))
        );
        ...
        Tast_assignment* curr_assign = tast_assignment_new(
            ...
            tast_wrap_member_access_typed(curr_src)
        );
...
static Tast_function_def* rm_tuple_function_def_new(Env* env, Tast_function_decl* decl) {
    ...
    switch (struct_def_->type) {
        case TAST_STRUCT_DEF:
            ...
            base_lang_type = lang_type_wrap_struct_const(lang_type_struct_new(lang_type_atom_new(base.name, 0)));
            break;
        case TAST_SUM_DEF:
    ...
    Tast_symbol_typed* rtn_child = tast_symbol_typed_new(decl->pos, (Sym_typed_base) {
        .lang_type = new_var->lang_type,
        .name = new_var->name,
        .llvm_id = 0
    });

    Tast_return* new_rtn = tast_return_new(decl->pos, tast_wrap_symbol_typed(rtn_child), true);
...
static Tast_expr* rm_tuple_tuple_rhs(
    ...
    Lang_type lhs_lang_type = lang_type_wrap_struct_const(lang_type_struct_new(lang_type_atom_new(serialize_lang_type(env, lang_type_wrap_tuple_const(rhs->lang_type)), 0)));
    ...
    switch (struct_def_->type) {
        case TAST_STRUCT_DEF:
            for (size_t idx = 0; idx < members.info.count; idx++) {
                ...
                if (sym_tbl_add(&vec_at(&env->ancesters, 0)->symbol_table, tast_wrap_variable_def(new_param))) {
                    vec_append(&a_main, &new_params, new_param);
                }
                log(LOG_DEBUG, STR_VIEW_FMT, tast_variable_def_print(new_param));
...
static Tast_expr* rm_tuple_sum_lit_rhs(
    ...
    switch (struct_def_->type) {
        case TAST_STRUCT_DEF: {
            ...
            try(sym_tbl_add(&vec_at(&env->ancesters, 0)->symbol_table, tast_wrap_variable_def(new_tag)));
            ...
            try(sym_tbl_add(&vec_at(&env->ancesters, 0)->symbol_table, tast_wrap_variable_def(new_item)));
...
static Tast_expr* rm_tuple_expr_rhs(Env* env, Tast_expr* rhs, Pos assign_pos) {
    ...
    switch (rhs->type) {
        ...
        case TAST_INDEX_TYPED:
            return rhs;
        case TAST_LITERAL:
            return rm_tuple_literal_rhs(env, tast_unwrap_literal(rhs), assign_pos);
