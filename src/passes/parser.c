...
static PARSE_STATUS extract_function_declaration_common(
    ...
    *fun_decl = node_unwrap_function_declaration(node_new(tk_view_front(*tokens).pos, NODE_FUNCTION_DECLARATION));
    ...
    if (!symbol_add(env, node_wrap_function_declaration(*fun_decl))) {
        return msg_redefinition_of_symbol(env, node_wrap_function_declaration(*fun_decl));
    }
    if (!try_consume(NULL, tokens, TOKEN_OPEN_PAR)) {
...
static PARSE_STATUS extract_function_definition(Env* env, Node_function_definition** fun_def, Tk_view* tokens) {
    ...
    (*fun_def) = node_unwrap_function_definition(
        node_new(node_wrap_function_declaration(fun_decl)->pos, NODE_FUNCTION_DEFINITION)
    );
...
static PARSE_STATUS extract_for_with_condition(Env* env, Node_for_with_condition** for_new, Node_for_range* for_range, Tk_view* tokens) {
    *for_new = node_unwrap_for_with_condition(node_new(node_wrap_for_range(for_range)->pos, NODE_FOR_WITH_CONDITION));
...
static PARSE_STATUS extract_for_loop(Env* env, Node** for_loop_result, Tk_view* tokens) {
    ...
    if (starts_with_variable_type_declaration(*tokens)) {
        ...
        *for_loop_result = node_wrap_for_with_condition(for_with_cond);
    }
...
static Node_literal* extract_literal(Env* env, Tk_view* tokens, bool defer_sym_add) {
    ...
    if (defer_sym_add) {
        symbol_add_defer(env, node_wrap_literal(new_node));
    } else {
        try(symbol_add(env, node_wrap_literal(new_node)));
    }
    return new_node;
}
...
static Node_symbol_untyped* extract_symbol(Tk_view* tokens) {
    ...
    assert(node_wrap_symbol_untyped(sym_node)->pos.line > 0);
    return sym_node;
}
...
static PARSE_STATUS try_extract_function_call(Env* env, Node_function_call** child, Tk_view* tokens) {
    ...
    if (!try_consume(NULL, &curr_tokens, TOKEN_CLOSE_PAR)) {
        ...
        msg(
            LOG_NOTE, EXPECT_FAIL_TYPE_NONE, node_wrap_function_call(function_call)->pos,
...
static PARSE_EXPR_STATUS extract_statement(Env* env, Node** child, Tk_view* tokens) {
    ...
    if (starts_with_struct_definition(*tokens)) {
        ...
        lhs = node_wrap_function_declaration(fun_decl);
        ...
        lhs = node_wrap_function_definition(fun_def);
        ...
        lhs = node_wrap_return_statement(rtn_statement);
...
static PARSE_STATUS extract_struct_literal(Env* env, Node_struct_literal** struct_lit, Tk_view* tokens) {
    ...
    *struct_lit = node_unwrap_struct_literal(
    ...
    try(symbol_add(env, node_wrap_struct_literal(*struct_lit)));
    return PARSE_OK;
}
...
static Node_struct_member_sym_untyped* extract_struct_member_call(Tk_view* tokens) {
    ...
    while (try_consume(NULL, tokens, TOKEN_SINGLE_DOT)) {
        ...
        vec_append(&a_main, &member_call->children, node_wrap_struct_member_sym_piece_untyped(member));
    }
    return member_call;
}
...
static PARSE_EXPR_STATUS try_extract_expression_piece(Env* env, Node** result, Tk_view* tokens, bool defer_sym_add) {
    ...
    if (try_consume(&open_par_token, tokens, TOKEN_OPEN_PAR)) {
        ...
        *result = node_wrap_operator(node_wrap_operator_generic(parser_unary_new(operator_token, inside_unary, unary_lang_type)));
        return PARSE_EXPR_OK;
        ...
        *result = node_wrap_variable_def(var_def);
        return PARSE_EXPR_OK;
        ...
        *result = node_wrap_struct_member_sym_untyped(extract_struct_member_call(tokens));
        return PARSE_EXPR_OK;
...
static PARSE_EXPR_STATUS try_extract_expression(Env* env, Node** result, Tk_view* tokens, bool defer_sym_add) {
    ...
    while (tokens->count > 0 && token_is_operator(tk_view_front(*tokens)) && !is_unary(tk_view_front(*tokens).type)) {
        ...
        if (is_unary(operator_token.type)) {
            ...
            node_auto_unwrap_op_binary(expression)->rhs = node_wrap_operator(node_wrap_operator_generic(operator));
        } else {
            Node* lhs = expression;
            Node* rhs;
            switch (try_extract_expression_piece(env, &rhs, tokens, defer_sym_add)) {
