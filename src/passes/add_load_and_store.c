...
static Llvm_register_sym load_expr(Env* env, Llvm_block* new_block, Node_expr* old_expr);

static Llvm_register_sym load_ptr(Env* env, Llvm_block* new_block, Node* old_node);

static Llvm_register_sym load_ptr_expr(Env* env, Llvm_block* new_block, Node_expr* old_expr);

static Llvm_register_sym load_node(Env* env, Llvm_block* new_block, Node* old_node);
...
static Llvm_register_sym load_binary(
    ...
    Llvm_llvm_placeholder* lhs = llvm_llvm_placeholder_new(old_bin->pos);
    lhs->lang_type = node_get_lang_type_expr(old_bin->lhs);
    lhs->llvm_reg = load_expr(env, new_block, old_bin->lhs);
    new_bin->lhs = llvm_wrap_llvm_placeholder(lhs);

    Llvm_llvm_placeholder* rhs = llvm_llvm_placeholder_new(old_bin->pos);
    rhs->lang_type = node_get_lang_type_expr(old_bin->rhs);
    rhs->llvm_reg = load_expr(env, new_block, old_bin->rhs);
    new_bin->rhs = llvm_wrap_llvm_placeholder(rhs);
...
static Llvm_register_sym load_unary(
    ...
    switch (old_unary->token_type) {
        ...
        case TOKEN_NOT: {
            ...
            Llvm_llvm_placeholder* child = llvm_llvm_placeholder_new(old_unary->pos);
            child->lang_type = node_get_lang_type_expr(old_unary->child);
            child->llvm_reg = load_expr(env, new_block, old_unary->child);
            new_unary->child = llvm_wrap_llvm_placeholder(child);
...
static Llvm_register_sym load_function_parameters(
    Env* env,
...
static Llvm_register_sym load_ptr_operator(Env* env, Llvm_block* new_block, Node_operator* old_oper) {
    switch (old_oper->type) {
        case NODE_BINARY:
            todo();
        case NODE_UNARY:
...
static Llvm_register_sym load_ptr_def(Env* env, Llvm_block* new_block, Node_def* old_def) {
    switch (old_def->type) {
...
static Llvm_register_sym load_def(Env* env, Llvm_block* new_block, Node_def* old_def) {
    switch (old_def->type) {
