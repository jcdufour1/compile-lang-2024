...
static Str_view load_function_call(
    ...
    unwrap(alloca_add(env, llvm_expr_wrap(llvm_function_call_wrap(new_fun_call))));
...
static Str_view load_raw_union_lit(
    ...
    Tast_member_access* access = tast_member_access_new(
        new_var->pos,
        vec_at(&union_def->base.members, (size_t)old_lit->tag->data)->lang_type,
        vec_at(&union_def->base.members, (size_t)old_lit->tag->data)->name,
...
static Llvm_block* for_range_to_branch(Env* env, Tast_for_range* old_for) {
    ...
    //unwrap(symbol_add(env, tast_variable_def_wrap(for_var_def)));
    Llvm* dummy = NULL;
    Tast_def* dummy_def = NULL;
    ...
    unwrap(try_set_binary_types_finish(env, &operator_, lhs_typed_, rhs_actual, old_for->pos, BINARY_LESS_THAN)); 
