...
Node_alloca* add_load_and_store_alloca_new(Env* env, Node_variable_def* var_def) {
    Node_alloca* alloca = node_alloca_new(var_def->pos);
    alloca->name = var_def->name;
    alloca->lang_type = var_def->lang_type;
    alloca_add(env, node_wrap_alloca(alloca));
    assert(alloca);
    return alloca;
}
...
static void do_function_def_alloca(Env* env, Node_block* new_block, const Node_function_def* old_fun_def) {
    ...
    for (size_t idx = 0; idx < params->params.info.count; idx++) {
        ...
        if (lang_type_is_struct(env, param->lang_type)) {
            alloca_add(env, node_wrap_def(node_wrap_variable_def(param)));
        } else if (lang_type_is_enum(env, param->lang_type)) {
            vec_insert(&a_main, &new_block->children, 0, node_wrap_alloca(
...
static Llvm_register_sym load_function_call(
    ...
    Node_function_call* new_fun_call = node_function_call_new(old_fun_call->pos);
...
static Node_literal* node_literal_clone(Node_literal* old_lit) {
    Node_literal* new_lit = NULL;

    switch (old_lit->type) {
        case NODE_STRING: {
            Node_string* string = node_string_new(node_get_pos_literal(old_lit));
            new_lit = node_wrap_string(string);
            break;
        }
        case NODE_VOID: {
            Node_void* lang_void = node_void_new(node_get_pos_literal(old_lit));
            new_lit = node_wrap_void(lang_void);
            break;
        }
        case NODE_ENUM_LIT: {
            Node_enum_lit* enum_lit = node_enum_lit_new(node_get_pos_literal(old_lit));
            new_lit = node_wrap_enum_lit(enum_lit);
            break;
        }
        case NODE_CHAR: {
            Node_char* lang_char = node_char_new(node_get_pos_literal(old_lit));
            new_lit = node_wrap_char(lang_char);
            break;
        }
        case NODE_NUMBER: {
            Node_number* number = node_number_new(node_get_pos_literal(old_lit));
            new_lit = node_wrap_number(number);
            break;
        }
        default:
...
static Llvm_register_sym load_ptr_symbol_typed(
    ...
    if (!alloca_lookup(&alloca, env, var_def->name)) {
        alloca = load_alloca(env, new_block, add_load_and_store_alloca_new(env, var_def)).node;
    }
    ...
    return (Llvm_register_sym) {
        .lang_type = get_lang_type(alloca),
        .node = alloca
    };
}
...
static Llvm_register_sym load_binary(
    ...
    Node_binary* new_bin = node_binary_new(old_bin->pos);

    Node_llvm_placeholder* lhs = node_llvm_placeholder_new(old_bin->pos);
    ...
    Node_llvm_placeholder* rhs = node_llvm_placeholder_new(old_bin->pos);
    ...
    switch (old_unary->token_type) {
        case TOKEN_DEREF: {
            if (lang_type_is_struct(env, get_lang_type_expr(old_unary->child))) {
                ...
                Node_load_another_node* new_load = node_load_another_node_new(old_unary->pos);
        ...
        case TOKEN_UNSAFE_CAST:
            ...
            // fallthrough
        case TOKEN_NOT: {
            Node_unary* new_unary = node_unary_new(old_unary->pos);

            Node_llvm_placeholder* child = node_llvm_placeholder_new(old_unary->pos);
...
static Llvm_register_sym load_ptr_index_typed(
    ...
    Node_load_element_ptr* new_load = node_load_element_ptr_new(old_index->pos);
...
static Llvm_register_sym load_member_access_typed(
    ...
    Node_load_another_node* new_load = node_load_another_node_new(old_access->pos);
    new_load->node_src = ptr;
...
static Llvm_register_sym load_index_typed(
    ...
    Node_load_another_node* new_load = node_load_another_node_new(old_index->pos);
    new_load->node_src = ptr;
    new_load->lang_type = ptr.lang_type;
...
static Llvm_register_sym load_function_def(
    ...
    vec_append(&a_main, &new_block->children, node_wrap_def(node_wrap_function_def(new_fun_def)));
    return (Llvm_register_sym) {
        .lang_type = old_fun_def->declaration->return_type->lang_type,
        .node = node_wrap_def(node_wrap_function_def(new_fun_def))
    };
}
...
static Llvm_register_sym load_function_decl(
    ...
    vec_append(&a_main, &new_block->children, node_wrap_def(node_wrap_function_decl(new_fun_decl)));
    return (Llvm_register_sym) {
        .lang_type = old_fun_decl->return_type->lang_type,
        .node = node_wrap_def(node_wrap_function_decl(new_fun_decl))
    };
}
...
static Node_block* if_statement_to_branch(Env* env, Node_if* if_statement, Str_view next_if, Str_view after_chain) {
    ...
    Node_goto* jmp_to_after_chain = goto_new(after_chain, old_block->pos);
...
static Node_block* if_else_chain_to_branch(Env* env, Node_if_else_chain* if_else) {
    Node_block* new_block = node_block_new(if_else->pos);
...
static Node_block* for_range_to_branch(Env* env, Node_for_range* old_for) {
    ...
    Node_goto* jmp_to_check_cond_label = goto_new(check_cond_label, old_for->pos);
    Node_goto* jmp_to_assign = goto_new(assign_label, old_for->pos);
...
static Llvm_register_sym load_break(
    ...
    Node_goto* new_goto = node_goto_new(old_break->pos);
...
static Llvm_register_sym load_continue(
    ...
    Node_goto* new_goto = node_goto_new(old_continue->pos);
...
static Llvm_register_sym load_ptr_def(Env* env, Node_block* new_block, Node_def* old_def) {
    switch (old_def->type) {
...
static Llvm_register_sym load_def(Env* env, Node_block* new_block, Node_def* old_def) {
    switch (old_def->type) {
        ...
        case NODE_LABEL:
            return load_label(env, new_block, node_unwrap_label(old_def));
        case NODE_STRUCT_DEF:
