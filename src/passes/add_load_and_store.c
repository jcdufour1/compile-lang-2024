...
static Node_id store_new(Node_id var_def) {
    Node_id alloca = node_new();
    nodes_at(alloca)->type = NODE_STORE;
    nodes_at(alloca)->name = nodes_at(var_def)->name;
    return alloca;
}
        ...
        case NODE_ASSIGNMENT: {
            ...
            switch (nodes_at(fun_def_or_decl_or_block)->type) {
                case NODE_FUNCTION_DEFINITION: {
                    Node_id block_parent = fun_def_or_decl_or_block;
                    Node_id fun_def = block_parent;
                    Node_id block_start = nodes_get_child_of_type(fun_def, NODE_BLOCK);
                    Node_id new_alloca = alloca_new(var_def);

                    Node_id last_child;
                    if (!nodes_try_get_last_child_of_type(&last_child, block_start, NODE_ALLOCA)) {
                        last_child = nodes_get_child(block_start, 0);
                    }
                    nodes_insert_after(last_child, new_alloca);
                    return;
                }
                ...
                case NODE_BLOCK: {
                    Node_id new_alloca = alloca_new(var_def);
                    Node_id block_start = fun_def_or_decl_or_block;
