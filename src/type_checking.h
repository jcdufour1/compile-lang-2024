...
bool try_set_expr_types(const Env* env, Node_expr** new_node, Node_expr* expr);

// returns false if unsuccessful
bool try_set_binary_types(const Env* env, Node_expr** new_node, Node_binary* operator);

bool try_set_block_types(Env* env, Node_block** new_node, Node_block* node);
...
bool try_set_member_access_types(const Env* env, Node** new_node, Node_member_access_untyped* access);
