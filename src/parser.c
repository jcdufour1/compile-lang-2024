...
static PARSE_STATUS parse_for_range_internal(
    ...
    if (PARSE_EXPR_OK != parse_expr(&lower_bound, tokens, false, block_scope)) {
    ...
    switch (parse_expr(&upper_bound, tokens, false, block_scope)) {
        case PARSE_EXPR_OK:
            break;
        case PARSE_EXPR_ERROR:
            return PARSE_ERROR;
...
static PARSE_STATUS parse_break(Uast_break** new_break, Tk_view* tokens, Scope_id scope_id) {
    ...
    switch (parse_expr(&break_expr, tokens, false, scope_id)) {
        case PARSE_EXPR_OK:
            do_break_expr = true;
            break;
        case PARSE_EXPR_ERROR:
...
static PARSE_STATUS parse_function_call(Uast_function_call** child, Tk_view* tokens, Uast_expr* callee, Scope_id scope_id) {
    ...
    while (is_first_time || prev_is_comma) {
        Uast_expr* arg;
        switch (parse_expr(&arg, tokens, false, scope_id)) {
            case PARSE_EXPR_OK:
                assert(arg);
                vec_append(&a_main, &args, arg);
...
static PARSE_STATUS parse_function_return(Uast_return** rtn_stmt, Tk_view* tokens, Scope_id scope_id) {
    ...
    Uast_expr* expr;
    switch (parse_expr(&expr, tokens, true, scope_id)) {
        case PARSE_EXPR_OK:
...
static PARSE_EXPR_STATUS parse_condition(Uast_condition** result, Tk_view* tokens, Scope_id scope_id) {
    ...
    switch (parse_expr(&cond_child, tokens, false, scope_id)) {
        case PARSE_EXPR_OK:
            break;
        case PARSE_EXPR_ERROR:
            return PARSE_EXPR_ERROR;
...
static PARSE_STATUS parse_switch(Uast_switch** lang_switch, Tk_view* tokens, Scope_id scope_id) {
    ...
    switch (parse_expr(&operand, tokens, false, scope_id)) {
        case PARSE_EXPR_OK:
            break;
        case PARSE_EXPR_ERROR:
            return PARSE_ERROR;
    ...
    while (1) {
        ...
        if (try_consume(NULL, tokens, TOKEN_CASE)) {
            switch (parse_expr(&case_operand, tokens, false, case_scope)) {
                case PARSE_EXPR_OK:
                    break;
                case PARSE_EXPR_ERROR:
                    return PARSE_ERROR;
...
static PARSE_EXPR_STATUS parse_stmt(Uast_stmt** child, Tk_view* tokens, Scope_id scope_id) {
    ...
    if (starts_with_type_def(*tokens)) {
        ...
        Uast_expr* lhs_ = NULL;
        switch (parse_expr(&lhs_, tokens, true, scope_id)) {
            case PARSE_EXPR_OK:
                break;
            case PARSE_EXPR_ERROR:
                return PARSE_EXPR_ERROR;
    ...
    if (try_consume(&equal_tk, tokens, TOKEN_SINGLE_EQUAL)) {
        Uast_expr* rhs = NULL;
        switch (parse_expr(&rhs, tokens, false, scope_id)) {
            case PARSE_EXPR_OK:
...
static PARSE_STATUS parse_struct_literal_members(Uast_expr_vec* members, Tk_view* tokens, Scope_id scope_id) {
    ...
    do {
        try_consume(NULL, tokens, TOKEN_NEW_LINE);
        Uast_expr* memb = NULL;
        switch (parse_expr(&memb, tokens, false, scope_id)) {
            case PARSE_EXPR_OK:
                break;
            case PARSE_EXPR_ERROR:
                return PARSE_ERROR;
...
static PARSE_EXPR_STATUS parse_expr_piece(
    ...
    if (try_consume(&open_par_token, tokens, TOKEN_OPEN_PAR)) {
        switch (parse_expr(result, tokens, false ,scope_id)) {
            case PARSE_EXPR_OK:
                break;
            case PARSE_EXPR_ERROR:
...
static PARSE_EXPR_STATUS parse_expr_index(
    ...
    switch (parse_expr(&index_index, tokens, prev_oper_pres, scope_id)) {
        case PARSE_EXPR_OK:
            break;
        case PARSE_EXPR_NONE:
            return PARSE_EXPR_NONE;
