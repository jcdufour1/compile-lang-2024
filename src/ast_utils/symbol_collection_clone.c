...
void usymbol_table_clone(Scope_id old_scope, Scope_id new_scope) {
    ...
    while (usym_tbl_iter_next(&curr, &iter)) {
        Uast_def* new_def = uast_def_clone(curr, true, new_scope);
...
void symbol_table_clone(Scope_id old_scope, Scope_id new_scope) {
    (void) new_scope;
    unwrap(darr_at_ref(&symbol_tables.symbol_table, old_scope)->hash_table.count == 0 && "not implemented; possibly should never be");
}

void alloca_table_clone(Scope_id old_scope, Scope_id new_scope) {
    (void) new_scope;
    unwrap(darr_at_ref(&symbol_tables.ir_table, old_scope)->hash_table.count == 0 && "not implemented; possibly should never be");
}


Scope_id scope_id_clone(Scope_id old_scope, Scope_id new_parent) {
    Scope_id new_scope = symbol_collection_new(new_parent, scope_to_name_tbl_lookup(old_scope));
