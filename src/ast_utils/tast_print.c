...
Strv tast_binary_print_internal(const Tast_binary* binary, Indent indent) {
    ...
    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(binary->lhs, indent));
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(binary->rhs, indent));
    indent -= INDENT_WIDTH;
...
Strv tast_unary_print_internal(const Tast_unary* unary, Indent indent) {
    ...
    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(unary->child, indent));
    indent -= INDENT_WIDTH;
...
Strv tast_member_access_print_internal(const Tast_member_access* access, Indent indent) {
    ...
    string_extend_strv_in_par(&a_temp, &buf, access->member_name);

    string_extend_cstr(&a_temp, &buf, "\n");

    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(access->callee, indent));
    indent -= INDENT_WIDTH;
...
Strv tast_index_print_internal(const Tast_index* index, Indent indent) {
    ...
    string_extend_cstr(&a_temp, &buf, "\n");
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(index->callee, indent + INDENT_WIDTH));
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(index->index, indent + INDENT_WIDTH));
...
Strv tast_function_call_print_internal(const Tast_function_call* fun_call, Indent indent) {
    ...
    string_extend_strv(&a_temp, &buf, lang_type_print_internal(LANG_TYPE_MODE_LOG, fun_call->lang_type));
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(fun_call->callee, indent + INDENT_WIDTH));

    for (size_t idx = 0; idx < fun_call->args.info.count; idx++) {
        Strv arg_text = tast_expr_print_internal(darr_at(fun_call->args, idx), indent + INDENT_WIDTH);
...
Strv tast_struct_literal_print_internal(const Tast_struct_literal* lit, Indent indent) {
    ...
    for (size_t idx = 0; idx < lit->members.info.count; idx++) {
        Strv memb_text = tast_expr_print_internal(darr_at(lit->members, idx), indent + INDENT_WIDTH);
...
Strv tast_tuple_print_internal(const Tast_tuple* lit, Indent indent) {
    ...
    string_extend_strv(&a_temp, &buf, lang_type_print_internal(LANG_TYPE_MODE_LOG, lang_type_tuple_const_wrap(lit->lang_type)));
...
Strv tast_enum_callee_print_internal(const Tast_enum_callee* lit, Indent indent) {
    ...
    string_extend_strv(&a_temp, &buf, lang_type_print_internal(LANG_TYPE_MODE_LOG, lit->enum_lang_type));
    string_extend_strv(&a_temp, &buf, tast_enum_tag_lit_print_internal(lit->tag, indent + INDENT_WIDTH));
...
Strv tast_enum_access_print_internal(const Tast_enum_access* lit, Indent indent) {
    ...
    string_extend_strv(&a_temp, &buf, lang_type_print_internal(LANG_TYPE_MODE_LOG, lit->lang_type));
    string_extend_strv(&a_temp, &buf, tast_enum_tag_lit_print_internal(lit->tag, indent + INDENT_WIDTH));
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(lit->callee, indent + INDENT_WIDTH));
...
Strv tast_enum_lit_print_internal(const Tast_enum_lit* lit, Indent indent) {
    ...
    string_extend_cstr(&a_temp, &buf, "\n");
    string_extend_strv(&a_temp, &buf, tast_enum_tag_lit_print_internal(lit->tag, indent + INDENT_WIDTH));
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(lit->item, indent + INDENT_WIDTH));
...
Strv tast_block_print_internal(const Tast_block* block, Indent indent) {
    ...
    string_extend_f(&a_temp, &buf, FMT, hash_table_stable_print_tast(darr_at_ref(&symbol_tables.symbol_table, block->scope_id), indent + INDENT_WIDTH, tast_def_print_internal));
...
Strv tast_function_params_print_internal(const Tast_function_params* function_params, Indent indent) {
    ...
    for (size_t idx = 0; idx < function_params->params.info.count; idx++) {
        Strv arg_text = tast_variable_def_print_internal(darr_at(function_params->params, idx), indent);
        string_extend_strv(&a_temp, &buf, arg_text);
    }
    indent -= INDENT_WIDTH;
...
Strv tast_condition_print_internal(const Tast_condition* cond, Indent indent) {
    ...
    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_operator_print_internal(cond->child, indent));
    indent -= INDENT_WIDTH;
...
Strv tast_for_with_cond_print_internal(const Tast_for_with_cond* for_cond, Indent indent) {
    ...
    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_condition_print_internal(for_cond->condition, indent));
    string_extend_strv(&a_temp, &buf, tast_block_print_internal(for_cond->body, indent));
    indent -= INDENT_WIDTH;
...
Strv tast_actual_break_print_internal(const Tast_actual_break* lang_break, Indent indent) {
    ...
    string_extend_cstr_indent(&a_temp, &buf, "break\n", indent);
...
Strv tast_yield_print_internal(const Tast_yield* yield, Indent indent) {
    ...
    if (yield->do_yield_expr) {
        string_extend_strv(&a_temp, &buf, tast_expr_print_internal(yield->yield_expr, indent + INDENT_WIDTH));
    }
...
Strv tast_assignment_print_internal(const Tast_assignment* assign, Indent indent) {
    ...
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(assign->lhs, indent + INDENT_WIDTH));
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(assign->rhs, indent + INDENT_WIDTH));
...
Strv tast_return_print_internal(const Tast_return* lang_rtn, Indent indent) {
    ...
    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_expr_print_internal(lang_rtn->child, indent));
    indent -= INDENT_WIDTH;
...
Strv tast_defer_print_internal(const Tast_defer* defer, Indent indent) {
    ...
    string_extend_strv(&a_temp, &buf, tast_stmt_print_internal(defer->child, indent + INDENT_WIDTH));
...
Strv tast_if_else_chain_print_internal(const Tast_if_else_chain* if_else, Indent indent) {
    ...
    for (size_t idx = 0; idx < if_else->tasts.info.count; idx++) {
        Strv arg_text = tast_if_print_internal(darr_at(if_else->tasts, idx), indent);
        string_extend_strv(&a_temp, &buf, arg_text);
    }
    indent -= INDENT_WIDTH;
...
Strv tast_module_alias_print_internal(const Tast_module_alias* alias, Indent indent) {
    ...
    string_extend_strv(&a_temp, &buf, alias->mod_path);
    string_extend_cstr(&a_temp, &buf, "\n");
...
Strv tast_function_decl_print_internal(const Tast_function_decl* fun_decl, Indent indent) {
    ...
    string_extend_cstr(&a_temp, &buf, "\n");
    string_extend_strv(&a_temp, &buf, tast_function_params_print_internal(fun_decl->params, indent));
    indent -= INDENT_WIDTH;
...
Strv tast_function_def_print_internal(const Tast_function_def* fun_def, Indent indent) {
    ...
    string_extend_cstr(&a_temp, &buf, "\n");
    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_function_decl_print_internal(fun_def->decl, indent));
    string_extend_strv(&a_temp, &buf, tast_block_print_internal(fun_def->body, indent));
    indent -= INDENT_WIDTH;
...
static void extend_struct_def_base(String* buf, const char* type_name, Struct_def_base base, Indent indent) {
    ...
    for (size_t idx = 0; idx < base.members.info.count; idx++) {
        Strv memb_text = tast_variable_def_print_internal(darr_at(base.members, idx), indent + INDENT_WIDTH);
        string_extend_strv(&a_temp, buf, memb_text);
    }
}
...
Strv tast_import_path_print_internal(const Tast_import_path* import, Indent indent) {
    ...
    indent += INDENT_WIDTH;
    string_extend_strv(&a_temp, &buf, tast_block_print_internal(import->block, indent));
    string_extend_cstr_indent(&a_temp, &buf, "mod_path: ", indent);
