...
Bytes alignof_ir_lang_type(Ir_lang_type lang_type) {
     switch (lang_type.type) {
         case IR_LANG_TYPE_PRIMITIVE: {
             // TODO: this may not work correctly with big ints if they use Lang_type_primitive
             Bytes size = sizeof_llvm_primitive(ir_lang_type_primitive_const_unwrap(lang_type));
...
// TODO: this function should call sizeof_prev_ir_params to reduce duplication?
Bytes sizeof_ir_params(Ir_variable_def_darr params) {
    Bytes sizeof_params = bytes_new(0);
    darr_foreach(idx, Ir_variable_def*, arg, params) {
        sizeof_params = bytes_add(sizeof_params, get_next_multiple(
            sizeof_ir_lang_type(arg->lang_type),
            bytes_new(8)/* TODO: this should vary depending on the backend */
        ));
    }
...
// key is excluded from size calculation
 Bytes sizeof_prev_ir_params(Ir_variable_def_darr params, Name key) {
     Bytes sizeof_prev = bytes_new(0);
    darr_foreach(idx, Ir_variable_def*, arg, params) {
        if (name_is_equal(arg->name_corr_param, key)) {
            break;
        }
        sizeof_prev = bytes_add(sizeof_prev, get_next_multiple(
            sizeof_ir_lang_type(arg->lang_type),
            bytes_new(8)/* TODO: this should vary depending on the backend */
        ));
    }
...
Bytes sizeof_ir_struct_def_base_internal(const Ir_struct_def_base* base, size_t idx_memb) {
    Bytes end_alignment = bytes_new(0);
...
Bytes sizeof_ir_struct_def_base(const Ir_struct_def_base* base) {
