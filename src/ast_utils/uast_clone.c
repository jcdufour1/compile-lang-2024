...
Uast_param_vec uast_param_vec_clone(Uast_param_vec vec, Scope_id new_scope) {
    Uast_param_vec new_vec = {0};
    for (size_t idx = 0; idx < vec.info.count; idx++) {
        vec_append(&a_main, &new_vec, uast_param_clone(vec_at(&vec, idx), new_scope));
    }
    return new_vec;
}

Uast_if_vec uast_if_vec_clone(Uast_if_vec vec, Scope_id new_scope, Pos dest_pos) {
    Uast_if_vec new_vec = {0};
    for (size_t idx = 0; idx < vec.info.count; idx++) {
        vec_append(&a_main, &new_vec, uast_if_clone(vec_at(&vec, idx), new_scope, dest_pos));
    }
    return new_vec;
}

Uast_expr_vec uast_expr_vec_clone(Uast_expr_vec vec, Scope_id new_scope, Pos dest_pos) {
    Uast_expr_vec new_vec = {0};
    for (size_t idx = 0; idx < vec.info.count; idx++) {
        vec_append(&a_main, &new_vec, uast_expr_clone(vec_at(&vec, idx), new_scope, dest_pos));
    }
    return new_vec;
}

Uast_case_vec uast_case_vec_clone(Uast_case_vec vec, Scope_id new_scope, Pos dest_pos) {
    Uast_case_vec new_vec = {0};
    for (size_t idx = 0; idx < vec.info.count; idx++) {
        vec_append(&a_main, &new_vec, uast_case_clone(vec_at(&vec, idx), new_scope, dest_pos));
    }
    return new_vec;
}
...
Uast_function_call* uast_function_call_clone(const Uast_function_call* fun_call, Scope_id new_scope, Pos dest_pos) {
    return uast_function_call_new(
        fun_call->pos,
        uast_expr_vec_clone(fun_call->args, new_scope, dest_pos),
        uast_expr_clone(fun_call->callee, new_scope, dest_pos)
    );
}
...
Uast_operator* uast_operator_clone(const Uast_operator* oper, Scope_id new_scope, Pos dest_pos) {
    switch (oper->type) {
        case UAST_UNARY:
            return uast_unary_wrap(uast_unary_clone(uast_unary_const_unwrap(oper), new_scope, dest_pos));
        case UAST_BINARY:
            return uast_binary_wrap(uast_binary_clone(uast_binary_const_unwrap(oper), new_scope, dest_pos));
    }
    unreachable("");
}
...
Uast_expr* uast_expr_clone(const Uast_expr* expr, Scope_id new_scope, Pos dest_pos) {
    switch (expr->type) {
        ...
        case UAST_OPERATOR:
            return uast_operator_wrap(uast_operator_clone(uast_operator_const_unwrap(expr), new_scope, dest_pos));
        case UAST_SYMBOL:
            return uast_symbol_wrap(uast_symbol_clone(uast_symbol_const_unwrap(expr), new_scope));
        case UAST_MEMBER_ACCESS:
            return uast_member_access_wrap(uast_member_access_clone(uast_member_access_const_unwrap(expr), new_scope, dest_pos));
        case UAST_INDEX:
            return uast_index_wrap(uast_index_clone(uast_index_const_unwrap(expr), new_scope, dest_pos));
        case UAST_LITERAL:
        ...
        case UAST_ENUM_GET_TAG: // TODO: remove uast_enum_access if not used
            todo();
        case UAST_SWITCH:
            return uast_switch_wrap(uast_switch_clone(uast_switch_const_unwrap(expr), new_scope, dest_pos));
...
Uast_def* uast_def_clone(const Uast_def* def, Scope_id new_scope) {
    switch (def->type) {
        ...
        case UAST_LABEL:
            return uast_label_wrap(uast_label_clone(uast_label_const_unwrap(def), new_scope));
    }
    unreachable("");
}

Uast_for_with_cond* uast_for_with_cond_clone(const Uast_for_with_cond* lang_for, Scope_id new_scope, Pos dest_pos) {
...
Uast_return* uast_return_clone(const Uast_return* rtn, Scope_id new_scope, Pos dest_pos) {
    return uast_return_new(rtn->pos, uast_expr_clone(rtn->child, new_scope, dest_pos), rtn->is_auto_inserted);
}
...
Uast_block* uast_block_clone(const Uast_block* block, Scope_id parent, Pos dest_pos) {
    ...
    for (size_t idx = 0; idx < block->children.info.count; idx++) {
        vec_append(&a_main, &new_children, uast_stmt_clone(vec_at(&block->children, idx), new_scope, dest_pos));
    }
...
Uast_if* uast_if_clone(const Uast_if* lang_if, Scope_id new_scope, Pos dest_pos) {
    return uast_if_new(
        lang_if->pos,
        uast_condition_clone(lang_if->condition, new_scope, dest_pos),
        uast_block_clone(lang_if->body, new_scope, dest_pos)
    );
}
