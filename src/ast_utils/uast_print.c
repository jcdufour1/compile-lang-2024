...
Strv uast_unary_print_internal(UAST_MODE mode, const Uast_unary* unary, Indent indent) {
    ...
    switch (mode) {
        case UAST_LOG:
            ...
            string_extend_f(&a_temp, &buf, FMT"\n", ulang_type_print(LANG_TYPE_MODE_LOG, unary->lang_type));
...
Strv uast_block_print_internal(UAST_MODE mode, const Uast_block* block, Indent indent) {
    ...
    switch (mode) {
        case UAST_LOG:
            ...
            indent += INDENT_WIDTH;
            extend_scope(&buf, block->scope_id, indent);
            string_extend_cstr(&a_temp, &buf, "\n");
            ...
            string_extend_f(&a_temp, &buf, FMT, hash_table_stable_print_uast(darr_at_ref(&symbol_tables.usymbol_table, block->scope_id), indent + INDENT_WIDTH, uast_def_print_internal_simple));
            ...
            string_extend_f(&a_temp, &buf, FMT, hash_table_stable_print_ir(darr_at_ref(&symbol_tables.ir_table, block->scope_id), indent + INDENT_WIDTH, ir_print_internal));

            for (size_t idx = 0; idx < block->children.info.count; idx++) {
                Strv arg_text = uast_stmt_print_internal(mode, darr_at(block->children, idx), indent);
                string_extend_strv(&a_temp, &buf, arg_text);
            }
...
Strv uast_mod_alias_print_internal(UAST_MODE mode, const Uast_mod_alias* alias, Indent indent) {
    ...
    switch (mode) {
        case UAST_LOG:
            ...
            string_extend_cstr(&a_temp, &buf, ")");
            string_extend_f(&a_temp, &buf, FMT, loc_print(loc_get(alias)));
            string_extend_cstr(&a_temp, &buf, "\n");
            break;
        case UAST_MSG:
            msg_todo("", alias->pos);
...
Strv uast_generic_param_print_internal(UAST_MODE mode, const Uast_generic_param* param, Indent indent) {
    ...
    switch (mode) {
        case UAST_LOG:
            ...
            indent += INDENT_WIDTH;
            if (param->is_expr) {
                string_extend_f_indent(&a_temp, &buf, indent, "expr: "FMT"\n", ulang_type_print(LANG_TYPE_MODE_LOG, param->expr_lang_type));
            }
...
Strv uast_variable_def_print_internal(UAST_MODE mode, const Uast_variable_def* def, Indent indent) {
    ...
    switch (mode) {
        case UAST_LOG:
            ...
            extend_name(NAME_LOG, &buf, def->name);
            string_extend_f(&a_temp, &buf, FMT, loc_print(loc_get(def)));
            string_extend_cstr(&a_temp, &buf, "\n");
            break;
        case UAST_MSG:
            msg_todo("", def->pos);
