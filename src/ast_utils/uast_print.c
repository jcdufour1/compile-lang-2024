...
Strv uast_block_print_internal(UAST_MODE mode, const Uast_block* block, Indent indent) {
    ...
    switch (mode) {
        case UAST_LOG:
            ...
            string_extend_f(&a_temp, &buf, FMT, hash_table_stable_print_uast(darr_at_ref(&symbol_tables.usymbol_table, block->scope_id), uast_def_print_internal_simple));

            string_extend_cstr_indent(&a_temp, &buf, "symbol_table\n", indent + INDENT_WIDTH);
            string_extend_f(&a_temp, &buf, FMT, hash_table_stable_print_tast(darr_at_ref(&symbol_tables.symbol_table, block->scope_id), tast_def_print_internal));

            string_extend_cstr_indent(&a_temp, &buf, "alloca_table\n", indent + INDENT_WIDTH);
            string_extend_f(&a_temp, &buf, FMT, hash_table_stable_print_ir(darr_at_ref(&symbol_tables.ir_table, block->scope_id), ir_print_internal));
