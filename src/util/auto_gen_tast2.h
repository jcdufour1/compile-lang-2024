...
static Tast2_type tast2_gen_union_lit(const char* prefix) {
    const char* base_name = "union_lit";
    Tast2_type union_lit = {.name = tast2_name_new(prefix, base_name, false)};

    append_member(&union_lit.members, "Tast2_enum_lit*", "tag");
    append_member(&union_lit.members, "Lang_type", "lang_type");
    append_member(&union_lit.members, "Tast2_expr*", "item");
...
static Tast2_type tast2_gen_literal(const char* prefix) {
    ...
    vec_append(&gen_a, &lit.sub_types, tast2_gen_union_lit(base_name));
...
static void tast2_gen_internal_unwrap(Tast2_type type, bool is_const) {
    ...
    //    try(tast2->type == upper); 
    string_extend_cstr(&gen_a, &function, "    try(tast2->type == ");
