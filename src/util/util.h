...
#define INTERPRET_STACK_SIZE bytes_new_macro(1024*1024UL) // TODO: use macro instead of bytes_new?
...
#define bytes_new_macro(num) ((Bytes) {.value = num})

static inline Bytes bytes_new(uint64_t value) {
    return bytes_new_macro(value);
}
...
static inline bool bytes_is_less_than(Bytes lhs, Bytes rhs) {
    return lhs.value < rhs.value;
}

static inline bool bytes_is_less_or_equal(Bytes lhs, Bytes rhs) {
    return lhs.value <= rhs.value;
}

static inline bool bytes_is_greater_or_equal(Bytes lhs, Bytes rhs) {
    return lhs.value >= rhs.value;
}
