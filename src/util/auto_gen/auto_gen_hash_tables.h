...
static void gen_hash_table(
    ...
    if (implementation) {
        ...
        if (do_scoped_lookup) {
            ...
            gen_gen("    Hash_table_scoped_"FMT"* collection,\n", strv_print(suffix));
            gen_gen("    Indent indent,\n");
            gen_gen("    Strv(*item_print_internal_fn)(const "FMT" item, Indent indent)\n", strv_print(type_with_ptr));
            gen_gen(") {\n");
            gen_gen("    String buf = {0};\n");
            ...
            gen_gen("    Strv curr_key = {0};\n");
            gen_gen("    "FMT" curr_item = {0};\n", strv_print(type_with_ptr));
            gen_gen("    Hash_table_scoped_iter_node_"FMT" iter = {0};\n", strv_print(suffix));
            gen_gen("    while (hash_table_scoped_iter_"FMT"(", strv_print(suffix));
            gen_gen("        collection,\n");
            gen_gen("        &curr_key,\n");
            gen_gen("        &curr_item,\n");
            gen_gen("        &iter\n");
            gen_gen("    )) {\n");
            gen_gen("        string_extend_f(&a_temp, &buf, FMT\"\\n\", strv_print(item_print_internal_fn(curr_item, indent)));\n");
            gen_gen("    }\n");
            gen_gen("\n");
            gen_gen("    return string_to_strv(buf);\n");
            gen_gen("}\n\n");
            gen_gen("#define hash_table_scoped_print_"FMT"(hash_table, scope_id, item_print_internal_fn) strv_print(hash_table_scoped_print_internal_"FMT"(hash_table, scope_id, item_print_internal_fn))\n\n", strv_print(suffix), strv_print(suffix));
        }
    } else {
