...
static void ulang_type_gen_internal_unwrap(Uast_type type, bool is_const) {
    for (size_t idx = 0; idx < type.sub_types.info.count; idx++) {
        ulang_type_gen_internal_unwrap(darr_at(type.sub_types, idx), is_const);
    }
...
static void ulang_type_gen_internal_wrap(Uast_type type, bool is_const) {
    for (size_t idx = 0; idx < type.sub_types.info.count; idx++) {
        ulang_type_gen_internal_wrap(darr_at(type.sub_types, idx), is_const);
    }
...
static void gen_ulang_type_set_ptr_depth_common(Uast_type ulang_type, Strv op_str, Strv name_op_str, bool is_get) {
    for (size_t idx = 0; idx < ulang_type.sub_types.info.count; idx++) {
        gen_ulang_type_set_ptr_depth_common(darr_at(ulang_type.sub_types, idx), op_str, name_op_str, is_get);
    }
    ...
    if (ulang_type.sub_types.info.count > 0) {
        ...
        darr_foreach(idx, Uast_type, sub_type, ulang_type.sub_types) {
...
static void gen_ulang_type_get_pos(Uast_type type, bool implementation, bool is_ref) {
    for (size_t idx = 0; idx < type.sub_types.info.count; idx++) {
        gen_ulang_type_get_pos(darr_at(type.sub_types, idx), implementation, is_ref);
    }
...
static void gen_ulang_type_common(const char* file_path, Uast_type ulang_type) {
    ...
    Uast_type_darr type_darr = uast_get_type_darr(ulang_type);
    (void) type_darr;
    ...
    gen_ulang_type_get_pos(ulang_type, true, false);
    gen_ulang_type_get_pos(ulang_type, true, true);
