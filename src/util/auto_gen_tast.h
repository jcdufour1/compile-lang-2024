...
static Tast_type tast_gen_int(const char* prefix) {
...
static Tast_type tast_gen_raw_union_lit(const char* prefix) {
...
static void tast_gen_print_overloading(Tast_type_vec types) {
    String function = {0};

    string_extend_cstr(&gen_a, &function, "#define tast_print(tast) strv_print(tast_print_overload(tast, 0))\n");

    string_extend_cstr(&gen_a, &function, "#define tast_print_overload(tast, indent) _Generic ((tast), \\\n");

    vec_foreach(idx, Tast_type, type, types) {
        string_extend_cstr(&gen_a, &function, "    ");
        extend_tast_name_first_upper(&function, type.name);
        string_extend_cstr(&gen_a, &function, "*: ");
        extend_tast_name_lower(&function, type.name);
    ...
    string_extend_cstr(&gen_a, &function, ") (tast, indent)\n");

    gen_gen(FMT"\n", string_print(function));
}

static void tast_get_type_vec_internal(Tast_type_vec* type_vec, Tast_type tast) {
    for (size_t idx = 0; idx < tast.sub_types.info.count; idx++) {
        tast_get_type_vec_internal(type_vec, vec_at(tast.sub_types, idx));
    }

    vec_append(&gen_a, type_vec, tast);
}

static Tast_type_vec tast_get_type_vec(Tast_type tast) {
    Tast_type_vec type_vec = {0};
    tast_get_type_vec_internal(&type_vec, tast);
    return type_vec;
}
...
static void gen_all_tasts(const char* file_path, bool implementation) {
    ...
    Tast_type_vec type_vec = tast_get_type_vec(tast);
    ...
    if (implementation) {
        tast_gen_print_overloading(type_vec);
