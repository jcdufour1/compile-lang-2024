...
static Node_type node_gen_block(void) {
    ...
    append_member(&block.members, "Node_vec", "children");
...
static Node_type node_gen_function_call(void) {
    ...
    append_member(&call.members, "Node_expr_vec", "args");
...
static Node_type node_gen_struct_lit_def(void) {
    ...
    append_member(&def.members, "Node_vec", "members");
    append_member(&def.members, "Str_view", "name");
    append_member(&def.members, "Lang_type", "lang_type");
...
static Node_type node_gen_if_else_chain(void) {
    ...
    append_member(&chain.members, "Node_if_vec", "nodes");
...
static void gen_node_vecs(Node_type node) {
    for (size_t idx = 0; idx < node.sub_types.info.count; idx++) {
        gen_node_vecs(vec_at(&node.sub_types, idx));
    }

    String vec_name = {0};
    extend_node_name_first_upper(&vec_name, node.name);
    string_extend_cstr(&gen_a, &vec_name, "_vec");

    String item_name = {0};
    extend_node_name_first_upper(&item_name, node.name);
...
static void gen_all_nodes(const char* file_path, bool implementation) {
    ...
    if (implementation) {
        ...
        gen_gen("#ifndef NODE_FORWARD_DECL_H\n");
        gen_gen("#define NODE_FORWARD_DECL_H\n");
    ...
    if (implementation) {
        ...
        gen_gen("#endif // NODE_FORWARD_DECL_H\n");
    }
