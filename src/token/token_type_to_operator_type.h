...
static inline BINARY_TYPE token_type_to_binary_type(TOKEN_TYPE token_type) {
    static_assert(TOKEN_COUNT == 73, "exhausive handling of token types (only binary tokens are handled)");
    switch (token_type) {
...
static inline UNARY_TYPE token_type_to_unary_type(TOKEN_TYPE token_type) {
    static_assert(TOKEN_COUNT == 73, "exhausive handling of token types (only unary tokens are handled)");
    switch (token_type) {
