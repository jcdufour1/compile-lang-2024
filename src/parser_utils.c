...
static bool is_function_definition(const Node* curr_node, const Node* var_call) {
...
static const Node* get_param_def_from_node_in_fun_body_common(const Node* node) {
    assert(node->name.count > 0);

    const Node* fun_def;
    if (!get_prev_matching_node(&fun_def, node, NULL, is_function_definition)) {
        unreachable("node "NODE_FMT" is not in function definition", node_print(node));
    }

    const Node* fun_params = nodes_get_child_of_type_const(fun_def, NODE_FUNCTION_PARAMETERS);
    size_t idx = 0;
    log(LOG_DEBUG, NODE_FMT"\n", node_print(fun_params->parent));
    nodes_foreach_child(param, fun_params) {
        if (str_view_is_equal(param->name, node->name)) {
            log(LOG_DEBUG, NODE_FMT"\n", node_print(param));
            return param;
        }
        idx++;
...
const Node* get_param_def_from_node_in_fun_body(const Node* node) {
    const Node* param = get_param_def_from_node_in_fun_body_common(node);
    if (!param) {
...
bool param_def_from_node_in_fun_body_exists(const Node* node) {
    return get_param_def_from_node_in_fun_body_common(node) ? true : false;
}
