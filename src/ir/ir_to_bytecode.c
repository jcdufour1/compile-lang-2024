...
static void ir_to_bytecode_return(Ir_return* rtn) {
    ...
    if (0 && bytecode_is_backpatching) {
        ...
        bytecode_append_return(sizeof_rtn_type, bytecode_stack_offset_);
...
uint64_t ir_to_bytecode_alloc_internal(uint64_t alloc_size, bool is_actual_push, bool should_emit_push) {
    ...
    if (is_actual_push) {
        ...
        if (should_emit_push) {
            bytecode_append_alloca(alloc_size, bytecode_stack_offset_);
        }
...
static void ir_to_bytecode_alloca(Ir_alloca* lang_alloca, bool is_for_local_var) {
    ...
    bytecode_append_alloca(sizeof_alloca, bytecode_stack_offset_);
...
static void ir_to_bytecode_load_another_ir(Ir_load_another_ir* load) {
    ...
    if (0 && bytecode_is_backpatching) {
        ...
        bytecode_append_store_stack(dest_pos, load_src_pos, sizeof_lang_type, bytecode_stack_offset_);
    }
...
static uint64_t ir_to_bytecode_push_variable_def(Ir_variable_def* var_def, bool is_from_rtn /* TODO: remove */) {
    ...
    if (is_from_rtn) {
        ...
        //uint64_t sizeof_load = sizeof_ir_lang_type(var_def->lang_type);
        todo();
        //bytecode_append_alloca(sizeof_load);
        todo();
        //uint64_t alloca_pos = bytecode_stack_size_sub_aligned(&bytecode_stack_offset, sizeof_load);

        //bytecode_append_store_stack(alloca_pos, store_src, sizeof_load);
        todo();
    }
...
static uint64_t ir_to_bytecode_push_unsafe_cast(Ir_unary* unary, bool is_actual_push) {
    ...
    return ir_to_bytecode_push_internal(sizeof_ir_lang_type(int_thing->lang_type), unary_pos, is_actual_push);
}
...
static uint64_t ir_to_bytecode_push_binary(Ir_binary* bin, bool is_actual_push) {
    ...
    return ir_to_bytecode_push_internal(sizeof_ir_lang_type(ir_get_lang_type(bin_pos_)), bin_pos, is_actual_push);
}
...
static uint64_t ir_to_bytecode_push_function_call(Ir_function_call* call, bool is_actual_push) {
    ...
    return ir_to_bytecode_push_internal(sizeof_ir_lang_type(ir_get_lang_type(call_pos_)), call_pos, is_actual_push);
}
...
static void ir_to_bytecode_store_another_ir(Ir_store_another_ir* store) {
    ...
    if (0 && bytecode_is_backpatching) {
        ...
        bytecode_append_store_stack(store_dest_pos, store_src_pos, sizeof_lang_type, bytecode_stack_offset_);
    }
...
static void ir_to_bytecode_function_call(Ir_function_call* call) {
    ...
    if (bytecode_is_backpatching) {
        ...
        old_count = bytecode.code.info.count;
        bytecode_append_call_direct(0, 0, 0, 0);
    } else {
        ...
        Bytecode_state bytecode_state = bytecode_state_save(bytecode_stack_offset_, bytecode.code.info.count);

        // TODO: remove below code (get arg_bytes_lower a different way)?
        uint64_t arg_bytes_lower = bytecode_stack_offset_;
        breakpoint();
        darr_foreach(idx, Name, arg, call->args) {
            ir_to_bytecode_push_ir(ir_from_name(arg), true);
        }
        ...
        old_count = bytecode.code.info.count;
        bytecode_append_call_direct(
            (uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(fun_addr_)))->data,
            get_next_multiple(arg_bytes_count, 8 /* alignment */),
            alloca_pos,
            bytecode_stack_offset_
        );
    }
