...
Bytes bytecode_stack_offset_ = bytes_new_macro(0);
Bytes bytecode_locals_offset = bytes_new_macro(0);
Bytes bytecode_locals_pos = bytes_new_macro(0);
Bytes bytecode_locals_pos_at_max_alloc = bytes_new_macro(0);
Bytes bytecode_space_locals_alloca_size = bytes_new_macro(0); // TODO: rename
...
static void ir_to_bytecode_bytes(Bytes value) {
    ir_to_bytecode_uint64_t(value.value);
}
...
static void ir_to_bytecode_append_store_stack_dir_addr(Bytes dest, int64_t src_value, Bytes sizeof_data, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;

    bytecode_append_align(BYTECODE_STORE_STACK_DIR_ADDR);
    ir_to_bytecode_bytes(dest);
    ir_to_bytecode_int64_t(src_value);
    ir_to_bytecode_bytes(sizeof_data);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_STORE_STACK_DIR_ADDR_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_store_stack(Bytes dest, Bytes src, Bytes sizeof_data, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;

    bytecode_append_align(BYTECODE_STORE_STACK);
    ir_to_bytecode_bytes(dest);
    ir_to_bytecode_bytes(src);
    ir_to_bytecode_bytes(sizeof_data);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_STORE_STACK_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_store_stack_deref_dest(Bytes dest, Bytes src, Bytes sizeof_data, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;
    ...
    assert(bytecode.code.info.count - old_count == BYTECODE_STORE_STACK_DEREF_DEST_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_deref(Bytes src, Bytes sizeof_alloca, Bytes alloca_pos, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;

    bytecode_append_align(BYTECODE_DEREF);
    ir_to_bytecode_bytes(src);
    ir_to_bytecode_bytes(sizeof_alloca);
    ir_to_bytecode_bytes(alloca_pos);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_DEREF_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_alloca(Bytes sizeof_data, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;
    ...
    assert(bytecode.code.info.count - old_count == BYTECODE_ALLOCA_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_call_direct(Bytes addr, Bytes arg_bytes, Bytes rtn_alloc_pos, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;

    bytecode_append_align(BYTECODE_CALL_DIRECT);
    ir_to_bytecode_bytes(addr);
    ir_to_bytecode_bytes(arg_bytes);
    ir_to_bytecode_bytes(rtn_alloc_pos);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_CALL_DIRECT_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_goto(Bytes addr, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;
    ...
    assert(bytecode.code.info.count - old_count == BYTECODE_GOTO_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_cond_goto(Bytes if_true, Bytes if_false, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;

    bytecode_append_align(BYTECODE_COND_GOTO);
    ir_to_bytecode_bytes(if_true);
    ir_to_bytecode_bytes(if_false);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_COND_GOTO_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_return(Bytes sizeof_rtn, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;

    bytecode_append_align(BYTECODE_RETURN);
    ir_to_bytecode_bytes(sizeof_rtn);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_RETURN_SIZE.value/*TODO*/);
}

static void ir_to_bytecode_append_zero_extend(Bytes sizeof_dest, Bytes sizeof_src, Bytes src_pos, Bytes alloca_pos, Bytes stack_offset_after) {
    uint64_t old_count = bytecode.code.info.count;

    bytecode_append_align(BYTECODE_ZERO_EXTEND);
    ir_to_bytecode_bytes(sizeof_dest);
    ir_to_bytecode_bytes(sizeof_src);
    ir_to_bytecode_bytes(src_pos);
    ir_to_bytecode_bytes(alloca_pos);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_ZERO_EXTEND_SIZE.value/*TODO*/);
}
...
static void ir_to_bytecode_append_binary(BYTECODE bin_type, Bytes start_args, Bytes lhs_pos, Bytes rhs_pos, Bytes alloca_pos, Bytes sizeof_bin_lang_type, Bytes stack_offset_after) {
    ...
    ir_to_bytecode_bytes(start_args);
    ir_to_bytecode_bytes(alloca_pos);
    ir_to_bytecode_bytes(sizeof_bin_lang_type);
    ir_to_bytecode_bytes(stack_offset_after);

    assert(bytecode.code.info.count - old_count == BYTECODE_BINARY_SIZE.value/*TODO*/);
}
...
static Bytes ir_to_bytecode_push_ir(Ir* store_src, bool is_actual_push, bool is_ptr);

static void ir_to_bytecode_expr_inline(Ir_expr* expr);

static void ir_to_bytecode_unary(Ir_unary* unary);

static Bytes ir_to_bytecode_pop_internal(Bytes sizeof_pop);
...
static void ir_to_bytecode_goto(Ir_goto* lang_goto) {
    ...
    if (bytecode_is_backpatching) {
        ir_to_bytecode_append_goto(bytes_new(0), bytes_new(0));
    } else {
        ...
        if (!bytes_is_equal(bytecode_stack_offset_,  bytecode_locals_pos_at_max_alloc)) {
            bytecode_dump(stderr, LOG_DEBUG, false, bytecode);
            log(LOG_DEBUG, "bytecode_stack_offset_ = "FMT", bytecode_locals_pos_at_max_alloc = "FMT"\n", bytes_print(bytecode_stack_offset_), bytes_print(bytecode_locals_pos_at_max_alloc));
            unreachable(
        ...
        ir_to_bytecode_append_goto(
            bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(block_pos_name)))->data),
...
static void ir_to_bytecode_cond_goto(Ir_cond_goto* cond_goto) {
    ...
    if (bytecode_is_backpatching) {
        ir_to_bytecode_pop_internal(bytes_new(1));

        ir_to_bytecode_append_cond_goto(bytes_new(0), bytes_new(0), bytecode_stack_offset_);
    } else {
        ...
        ir_to_bytecode_append_cond_goto(
            bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(if_true_pos_name)))->data),
            bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(if_false_pos_name)))->data),
            bytecode_stack_offset_
        );
    }
...
static void ir_to_bytecode_return(Ir_return* rtn) {
    ...
    if (0 && bytecode_is_backpatching) {
        ...
        log(LOG_DEBUG, "bytecode_space_locals_alloca_size = "FMT"\n", bytes_print(bytecode_space_locals_alloca_size));
        log(LOG_DEBUG, "bytecode_stack_offset = "FMT"\n", bytes_print(bytecode_stack_offset_));
        //breakpoint();
        if (bytecode_is_backpatching) {
            unwrap(ir_add(ir_expr_wrap(ir_literal_wrap(ir_int_wrap(ir_int_new(
                bytecode_fun_pos,
                (int64_t)bytecode_space_locals_alloca_size.value/*TODO*/,
        ...
        Bytes sizeof_rtn_type = sizeof_ir_lang_type(ir_get_lang_type(rtn_child));
...
Bytes ir_to_bytecode_alloc_internal(Bytes alloc_size, bool is_actual_push, bool should_emit_push) {
    assert(bytes_is_equal(get_next_multiple(bytecode_stack_offset_, bytes_new(8)), bytecode_stack_offset_) && "not implemented");
    assert(bytes_is_equal(get_next_multiple(bytecode_locals_offset, bytes_new(8)), bytecode_locals_offset) && "not implemented");

    if (is_actual_push) {
        assert(bytes_is_less_than(bytecode_stack_offset_, bytes_new(1000)));
        bytecode_stack_offset_ = bytes_add(bytecode_stack_offset_, get_next_multiple(alloc_size, bytes_new(8)));
        bytecode_stack_offset_ = get_next_multiple(bytecode_stack_offset_, bytes_new(8));
        log(LOG_DEBUG, FMT"\n", bytes_print(bytecode_stack_offset_));
    ...
    bytecode_locals_offset = bytes_add(bytecode_locals_offset, get_next_multiple(alloc_size, bytes_new(8)));
    bytecode_space_locals_alloca_size = bytes_add(bytecode_space_locals_alloca_size, get_next_multiple(alloc_size, bytes_new(8)));
    //assert(bytecode_locals_pos < 1000);
    //assert(bytecode_space_locals_alloca_size < 1000);
    return bytes_add(bytecode_locals_pos, bytecode_space_locals_alloca_size);
}
...
static void ir_to_bytecode_alloca(Ir_alloca* lang_alloca, bool is_for_local_var) {
    size_t old_count = 0;
    //breakpoint();
    Bytes sizeof_alloca = sizeof_ir_lang_type(ir_lang_type_pointer_depth_dec(lang_alloca->lang_type));

    if (is_for_local_var) {
        old_count = bytecode.code.info.count;
        //assert(bytecode_stack_offset_ < 1000);
        Bytes alloca_pos = ir_to_bytecode_alloc_internal(sizeof_alloca, false/*TODO*/, false);
        //assert(bytecode_stack_offset_ < 1000);
        ...
        if (bytecode_is_backpatching) {
            unwrap(ir_add(ir_expr_wrap(ir_literal_wrap(ir_int_wrap(ir_int_new(
                ...
                ir_lang_type_new_ux(bytes_to_bits(sizeof_alloca)),
    ...
    {
        Bytes alloca_pos = bytecode_stack_size_sub_aligned(&bytecode_stack_offset_, sizeof_alloca);
...
static Bytes ir_to_bytecode_push_int_internal(const char* file, int line, Ir_int* lang_int, bool is_actual_push, bool is_ptr) {
    assert(!is_ptr && "not implemented");
    size_t old_count = bytecode.code.info.count;
    log(LOG_DEBUG, "ir_to_bytecode_push_int: old_count = %zu\n", old_count);
    Bytes sizeof_int = sizeof_ir_lang_type(lang_int->lang_type);
    //assert(bytecode_stack_offset_ < 1000);
    log(LOG_DEBUG, "bytecode_stack_offset_ = "FMT"\n", bytes_print(bytecode_stack_offset_));
    Bytes int_pos = ir_to_bytecode_alloc_internal(sizeof_int, is_actual_push, true);
    //assert(bytecode_stack_offset_ < 1000);

    assert(bytes_is_less_or_equal(sizeof_int, bytes_new(8)));
    // TODO: always call function similar to this
    ir_to_bytecode_comment("push int store stack");
    ir_to_bytecode_comment_internal(file, line, "    (called from)");
    ...
    log(LOG_DEBUG, FMT" %"PRIi64" "FMT"\n", bytes_print(int_pos), lang_int->data, bytes_print(sizeof_int));
...
#define ir_to_bytecode_push_int(lang_int, is_actual_push, is_ptr) \
    ir_to_bytecode_push_int_internal(__FILE__, __LINE__, lang_int, is_actual_push, is_ptr)

// TODO: move bool_print
#define bool_print(val) strv_print((val) ? sv("true") : sv("false"))

static Bytes ir_to_bytecode_push_internal_2(const char* file, int line, Bytes sizeof_load, Bytes src_pos, bool is_actual_push, bool is_ptr) {
    if (!is_actual_push) {
        if (is_ptr) {
            todo();
        }
    ...
    //assert(bytecode_stack_offset_ < 1000);
    Bytes alloca_pos = ir_to_bytecode_alloc_internal(sizeof_load, is_actual_push, true);
    //assert(bytecode_stack_offset_ < 1000);

    if (is_ptr) {
        assert(bytes_is_equal(sizeof_load, bytes_new(8)) && "pointers should always be 8 bytes");
        log(LOG_DEBUG, "inner thing =  "FMT"\n", bytes_print(src_pos));
        ir_to_bytecode_comment("push internal if store stack (is_actual_push = "FMT", is_ptr = "FMT")", bool_print(is_actual_push), bool_print(is_ptr));
        ir_to_bytecode_comment_internal(file, line, "    (called from)");
        ir_to_bytecode_append_store_stack_dir_addr(alloca_pos, (int64_t)src_pos.value/*TODO*/, sizeof_load, bytecode_stack_offset_);
        bytecode_dump(stderr, LOG_DEBUG, true, bytecode);
        //breakpoint();

    } else {
        ir_to_bytecode_comment("push internal else store stack (is_actual_push = "FMT", is_ptr = "FMT", alloca_pos = "FMT", src_pos = "FMT")", bool_print(is_actual_push), bool_print(is_ptr), bytes_print(alloca_pos), bytes_print(src_pos));
        ir_to_bytecode_comment_internal(file, line, "    (called from)");
...
#define ir_to_bytecode_push_internal(sizeof_load, src_pos, is_actual_push, is_ptr) \
    ir_to_bytecode_push_internal_2(__FILE__, __LINE__, sizeof_load, src_pos, is_actual_push, is_ptr)
...
static Bytes ir_to_bytecode_pop_internal(Bytes sizeof_pop) {
    ...
    Bytes pos = bytecode_stack_offset_;
    bytecode_stack_offset_ = bytes_subtract(bytecode_stack_offset_, get_next_multiple(sizeof_pop, bytes_new(8)));
    return pos;
}
...
static void ir_to_bytecode_load_another_ir(Ir_load_another_ir* load) {
    ...
    if (0 && bytecode_is_backpatching) {
        ...
        Bytes sizeof_lang_type = sizeof_ir_lang_type(load->lang_type);
        //assert(bytecode_stack_offset_ < 1000);
        Bytes dest_pos = ir_to_bytecode_alloc_internal(sizeof_lang_type, false, true);
        //assert(bytecode_stack_offset_ < 1000);
        ...
        // struct is at 176
        // denominator is at 172
        if (load_src_pos.value == 172) {
        ...
        ir_add(ir_expr_wrap(ir_literal_wrap(ir_int_wrap(ir_int_new(
            load->pos,
            (int64_t)dest_pos.value/*TODO*/,
            ir_lang_type_new_ux(bits_new(64)/*TODO*/),
            symbol_name_to_int_name(load->name)
        )))));


        ir_to_bytecode_comment("load_another_ir end store stack");
        ir_to_bytecode_append_store_stack(dest_pos, load_src_pos, sizeof_lang_type, bytecode_stack_offset_);
    }
}
...
static Bytes ir_to_bytecode_push_alloca(Ir_alloca* lang_alloca, bool is_actual_push, bool is_ptr) {
    ...
    Bytes alloca_pos = bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(stack_pos_name)))->data);
    // TODO: avoid store just to 
...
static Bytes ir_to_bytecode_push_load_another_ir(Ir_load_another_ir* load, bool is_from_rtn /* TODO: remove */, bool is_actual_push, bool is_ptr) {
    assert(!is_ptr && "not implemented");
    ...
    Bytes sizeof_load = sizeof_ir_lang_type(load->lang_type);
...
static Bytes ir_to_bytecode_push_load_element_ptr(Ir_load_element_ptr* load, bool is_from_rtn /* TODO: remove */, bool is_actual_push, bool is_ptr) {
    assert(!is_ptr && "not implemented");
    (void) is_from_rtn; // TODO: remove
    Ir* stack_pos_name = NULL;
    ...
    log(LOG_DEBUG, FMT"\n", ir_print(load));
    Ir_lang_type elem_lang_type = ir_lang_type_pointer_depth_dec(load->lang_type);
    Bytes sizeof_load = sizeof_ir_lang_type(elem_lang_type);
    log(LOG_DEBUG, FMT"\n", ir_lang_type_print(LANG_TYPE_MODE_LOG, elem_lang_type));
    return ir_to_bytecode_push_internal(sizeof_load, elem_ptr, is_actual_push, is_ptr);
...
static Bytes ir_to_bytecode_push_variable_def(Ir_variable_def* var_def, bool is_from_rtn /* TODO: remove */) {
    // TODO: make function to do this calculation in sizeof.c?
    Bytes store_src = bytes_add(sizeof_prev_ir_params(curr_fun_args, var_def->name_corr_param), bytes_new(8));
...
static Bytes ir_to_bytecode_push_unsafe_cast(Ir_unary* unary, bool is_actual_push) {
    (void) is_actual_push;
...
// returns pos
static Bytes ir_to_bytecode_push_unary(Ir_unary* unary, bool is_actual_push, bool is_ptr) {
    assert(!is_ptr && "not implemented");
    assert(unary->token_type == IR_UNARY_UNSAFE_CAST);
...
// returns pos
static Bytes ir_to_bytecode_push_binary(Ir_binary* bin, bool is_actual_push, bool is_ptr) {
    assert(!is_ptr && "not implemented");
    Ir* bin_pos_ = NULL;
    unwrap(ir_lookup(&bin_pos_, symbol_name_to_int_name(bin->name)));
    Bytes bin_pos = bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(bin_pos_)))->data);
    ...
    //if (sizeof_ir_lang_type(ir_get_lang_type(bin_pos_)) > 8) {
        //breakpoint();
    //}
    assert(bytes_is_less_or_equal(sizeof_ir_lang_type(ir_get_lang_type(bin_pos_)), bytes_new(8)));
    return ir_to_bytecode_push_internal(sizeof_ir_lang_type(ir_get_lang_type(bin_pos_)), bin_pos, is_actual_push, is_ptr);
}

// returns pos
static Bytes ir_to_bytecode_push_function_call(Ir_function_call* call, bool is_actual_push, bool is_ptr) {
    assert(!is_ptr && "not implemented");
    Ir* call_pos_ = NULL;
    unwrap(ir_lookup(&call_pos_, symbol_name_to_int_name(call->name_self)));
    Bytes call_pos = bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(call_pos_)))->data);

    return ir_to_bytecode_push_internal(sizeof_ir_lang_type(ir_get_lang_type(call_pos_)), call_pos, is_actual_push, is_ptr);
}

// returns pos
static Bytes ir_to_bytecode_push_operator(Ir_operator* oper, bool is_actual_push, bool is_ptr) {
    switch (oper->type) {
        case IR_UNARY:
...
// returns pos
static Bytes ir_to_bytecode_push_expr(Ir_expr* expr, bool is_actual_push, bool is_ptr) {
    switch (expr->type) {
        case IR_OPERATOR:
...
static Bytes ir_to_bytecode_push_def(Ir_def* def, bool is_from_rtn, bool is_ptr) {
    assert(!is_ptr && "not implemented");
    switch (def->type) {
...
static Bytes ir_to_bytecode_store_dest(bool* deref_is_req, Bytes* sizeof_lang_type, Ir* store_dest) {
    *deref_is_req = false;
    ...
    switch (store_dest->type) {
        ...
        case IR_LOAD_ELEMENT_PTR: {
            ...
            *sizeof_lang_type = sizeof_ir_lang_type(int_lang_type);
            ...
            return bytes_new((uint64_t)lang_int->data);
...
static void ir_to_bytecode_store_another_ir(Ir_store_another_ir* store) {
    ...
    if (0 && bytecode_is_backpatching) {
        ...
        Bytes dummy = {0};
        Bytes sizeof_lang_type = sizeof_ir_lang_type(store->lang_type);
        log(LOG_DEBUG, FMT"\n", ir_print(store_src));
        log(LOG_DEBUG, FMT"\n", ir_print(store_dest));
        log(LOG_DEBUG, FMT"\n", bytes_print(bytecode_stack_offset_));
        Bytes store_src_pos = ir_to_bytecode_push_ir(store_src, false, false);
        if (bytes_is_greater_than(store_src_pos, bytes_new(120))) {
            //breakpoint();
        }
        log(LOG_DEBUG, FMT" "FMT"\n", bytes_print(bytecode_stack_offset_), bytes_print(store_src_pos));
        //assert(bytecode_stack_offset == store_src_pos);
        bool deref_is_req = {0};
        Bytes store_dest_pos = ir_to_bytecode_store_dest(&deref_is_req, &dummy, store_dest);
        ...
        if (deref_is_req) {
            if (!bytecode_is_backpatching) {
                log(LOG_DEBUG, "store_dest_pos = "FMT"\n", bytes_print(store_dest_pos));
                //todo();
                todo();
            }
            ir_to_bytecode_comment("store_another_ir 67 store stack");
            ir_to_bytecode_append_store_stack_deref_dest(store_dest_pos, store_src_pos, sizeof_lang_type, bytecode_stack_offset_);
        } else {
            ir_to_bytecode_comment("store_another_ir 68 store stack");
...
static void ir_to_bytecode_function_def(Ir_function_def* def) {
    ...
    bytecode_stack_offset_ = bytes_new(0);
    ...
    Bytes old_bytecode_space_locals_alloca_size = bytecode_space_locals_alloca_size;
    bytecode_space_locals_alloca_size = bytes_new(0);
    ...
    Bytes old_bytecode_stack_offset = bytecode_stack_offset_;
    bytecode_stack_offset_ = sizeof_ir_params(def->decl->params->params);
    bytecode_stack_offset_ = get_next_multiple(bytecode_stack_offset_, bytes_new(8));
    Bytes args_count_bytes = bytecode_stack_offset_;
    ...
    static_assert(BYTECODE_COUNT_RTN_ITEMS == 6, "exhausive handling of values below in allocs");
    {
        ...
        ir_to_bytecode_alloca(ir_alloca_new(def->pos, ir_lang_type_new_ux(bits_new(64)), util_literal_name_new(), 0 /* TODO */), false);
        assert(bytes_is_equal(bytecode_stack_offset_, bytecode_call_stack_get_offset(BYTECODE_CALL_STACK_TYPE_RTN_VALUE, args_count_bytes)));

        ir_to_bytecode_comment("alloca for return address");
        ir_to_bytecode_alloca(ir_alloca_new(def->pos, ir_lang_type_new_ux(bits_new(64)), util_literal_name_new(), 0 /* TODO */), false);
        assert(bytes_is_equal(bytecode_stack_offset_, bytecode_call_stack_get_offset(BYTECODE_CALL_STACK_TYPE_RTN_ADDR, args_count_bytes)));

        ir_to_bytecode_comment("alloca for stack_base_ptr");
        ir_to_bytecode_alloca(ir_alloca_new(def->pos, ir_lang_type_new_ux(bits_new(64)), util_literal_name_new(), 0 /* TODO */), false);
        assert(bytes_is_equal(bytecode_stack_offset_, bytecode_call_stack_get_offset(BYTECODE_CALL_STACK_TYPE_BASE_PTR, args_count_bytes)));

        ir_to_bytecode_comment("alloca for stack_offset");
        ir_to_bytecode_alloca(ir_alloca_new(def->pos, ir_lang_type_new_ux(bits_new(64)), util_literal_name_new(), 0 /* TODO */), false);
        assert(bytes_is_equal(bytecode_stack_offset_, bytecode_call_stack_get_offset(BYTECODE_CALL_STACK_TYPE_OFFSET, args_count_bytes)));

        ir_to_bytecode_comment("alloca for arg_bytes");
        ir_to_bytecode_alloca(ir_alloca_new(def->pos, ir_lang_type_new_ux(bits_new(64)), util_literal_name_new(), 0 /* TODO */), false);
        assert(bytes_is_equal(bytecode_stack_offset_, bytecode_call_stack_get_offset(BYTECODE_CALL_STACK_TYPE_ARG_BYTES, args_count_bytes)));

        ir_to_bytecode_comment("alloca for rtn_alloc_pos");
        ir_to_bytecode_alloca(ir_alloca_new(def->pos, ir_lang_type_new_ux(bits_new(64)), util_literal_name_new(), 0 /* TODO */), false);
        assert(bytes_is_equal(bytecode_stack_offset_, bytecode_call_stack_get_offset(BYTECODE_CALL_STACK_TYPE_RTN_ALLOC_POS, args_count_bytes)));
    }

    {
        Bytes space_locals_bytes = bytes_new(0);
        Bytes start_local_vars = bytecode_stack_offset_;
        if (!bytecode_is_backpatching) {
            Ir* space_locals_ = NULL;
            unwrap(ir_lookup(&space_locals_, fun_name_to_space_locals_alloc_size(bytecode_fun_name)));
            space_locals_bytes = bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(space_locals_)))->data);
        }
        ir_to_bytecode_comment("alloca for local variables");
        bytecode_locals_pos = start_local_vars;
        if (!bytecode_is_backpatching) {
            bytecode_locals_pos_at_max_alloc = bytes_add(bytecode_locals_pos, space_locals_bytes);
        }
...
static void ir_to_bytecode_function_call(Ir_function_call* call) {
    ...
    Bytes sizeof_lang_type = sizeof_ir_lang_type(call->lang_type);
        //assert(bytecode_stack_offset_ < 1000);
    //uint64_t temp_alloca_pos = ir_to_bytecode_alloc_internal(sizeof_lang_type, true, false);
        //assert(bytecode_stack_offset_ < 1000);
    //(void) temp_alloca_pos;
        //assert(bytecode_stack_offset_ < 1000);
    Bytes alloca_pos = ir_to_bytecode_alloc_internal(sizeof_lang_type, false, false);
        //assert(bytecode_stack_offset_ < 1000);
    if (0 && bytecode_is_backpatching) {
        ir_add(ir_expr_wrap(ir_literal_wrap(ir_int_wrap(ir_int_new(
            call->pos /* TODO*/,
        ...
        ir_to_bytecode_append_call_direct(bytes_new(0), bytes_new(0), bytes_new(0), bytes_new(0));
    } else {
        ...
        Ir* fun_addr_ = NULL;
        Bytes fun_addr = bytes_new(0);
        if (!bytecode_is_backpatching) {
            unwrap(ir_lookup(&fun_addr_, function_name_to_loc(fun_name_->fun_name)));
            fun_addr = bytes_new((uint64_t)ir_int_unwrap(ir_literal_unwrap(ir_expr_unwrap(fun_addr_)))->data);
        }

        Bytes arg_bytes_lower = bytecode_stack_offset_;
        ...
        Bytes arg_bytes_count = bytes_subtract(bytecode_stack_offset_, arg_bytes_lower);
        arg_bytes_count = get_next_multiple(arg_bytes_count, bytes_new(8) /* alignment */);
...
#   define dest_name (unary->name)
    ...
    Bytes sizeof_dest = sizeof_ir_lang_type(unary->lang_type);
    Bytes sizeof_src = sizeof_ir_lang_type(ir_get_lang_type(src));

    Bytes alloca_pos = ir_to_bytecode_alloc_internal(sizeof_dest, false, false);

    if (src_prim_type.type == IR_LANG_TYPE_UNSIGNED_INT && ir_lang_type_is_int(ir_get_lang_type(src))) {
        if (1 || bytecode_is_backpatching) {
            if (bytes_is_less_or_equal(sizeof_dest, sizeof_src)) {
                // not zero extend
                todo();
            }

            // TODO: remove 
            Bytes src_pos = ir_to_bytecode_push_ir(src, true, false);
            ...
            if (bytecode_is_backpatching) {
                unwrap(ir_add(ir_expr_wrap(ir_literal_wrap(ir_int_wrap(ir_int_new(
                    unary->pos /* TODO*/,
                    (int64_t)alloca_pos.value/*TODO*/,
                    ir_lang_type_new_ux(bytes_to_bits(sizeof_src)),
                    symbol_name_to_int_name(unary->name/*TODO*/)
                ))))));
            }
        } else {
...
static void ir_to_bytecode_binary(Ir_binary* bin) {
    ...
    Bytes lhs_pos = ir_to_bytecode_push_ir(ir_from_name(bin->lhs), true, false);
    //breakpoint();
    ...
    Bytes rhs_pos = ir_to_bytecode_push_ir(ir_from_name(bin->rhs), true, false);
    Bytes start_args = rhs_pos;
    assert(bytes_is_less_than(start_args, bytes_new(1000)));
    assert(bytes_is_less_than(rhs_pos, bytes_new(1000)));
    ir_to_bytecode_comment("add lhs: "FMT"\n", ir_print(ir_from_name(bin->lhs)));
    ir_to_bytecode_comment("add lhs pos: "FMT"\n", bytes_print(start_args));
    ir_to_bytecode_comment("add rhs: "FMT"\n", ir_print(ir_from_name(bin->rhs)));
    ir_to_bytecode_comment("add rhs pos: "FMT"\n", bytes_print(rhs_pos));
    if (!bytecode_is_backpatching) {
        assert(bytes_is_greater_than(rhs_pos, lhs_pos));
        assert(bytes_is_greater_than(start_args, lhs_pos));
    }

    Bytes sizeof_bin_lang_type = sizeof_ir_lang_type(bin->lang_type);

        //assert(bytecode_stack_offset_ < 1000);
    Bytes alloca_pos = ir_to_bytecode_alloc_internal(sizeof_bin_lang_type, false, false);
        //assert(bytecode_stack_offset_ < 1000);
