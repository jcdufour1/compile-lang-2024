...
Name lang_type_primitive_get_name(const Lang_type_primitive lang_type) {
    ...
    Name new_name = name_new(MOD_PATH_BUILTIN, new_base, (Ulang_type_darr) {0}, SCOPE_BUILTIN);
...
bool lang_type_get_name(Name* result, Lang_type lang_type) {
    switch (lang_type.type) {
        ...
        case LANG_TYPE_TUPLE:
            return false;
        case LANG_TYPE_VOID:
            *result = name_new(MOD_PATH_BUILTIN, sv("void"), (Ulang_type_darr) {0}, SCOPE_BUILTIN);
            return true;
        case LANG_TYPE_FN:
            return false;
