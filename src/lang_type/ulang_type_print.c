...
void extend_ulang_type_to_string(String* string, LANG_TYPE_MODE mode, Ulang_type lang_type) {
    switch (lang_type.type) {
        case ULANG_TYPE_ARRAY: {
            log(LOG_DEBUG, "ulang_type_array\n");
            ...
            string_extend_cstr(&a_temp, string, "]");
            log(LOG_DEBUG, "ulang_type_reg returing\n");
            return;
        }
        case ULANG_TYPE_REGULAR: {
            log(LOG_DEBUG, "ulang_type_regualr\n");
        ...
        case ULANG_TYPE_TUPLE: {
            ...
            for (size_t idx = 0; idx < tuple.ulang_types.info.count; idx++) {
                ...
                extend_ulang_type_to_string(string, mode, darr_at(tuple.ulang_types, idx));
            }
            darr_append(&a_temp, string, ')');
            log(LOG_DEBUG, "ulang_type_tuple returing\n");
            return;
        }
        case ULANG_TYPE_FN: {
            ...
            log(LOG_DEBUG, "ulang_type_fn returing\n");
            return;
        }
        case ULANG_TYPE_REMOVED: {
            log(LOG_DEBUG, "ulang_type_rem\n");
            string_extend_cstr(&a_main, string, "removed");
            log(LOG_DEBUG, "ulang_type_rem returing\n");
            return;
        }
        case ULANG_TYPE_EXPR: {
            log(LOG_DEBUG, "ulang_type_expr\n");
            ...
            log(LOG_DEBUG, "ulang_type_expr returning\n");
            return;
        }
        case ULANG_TYPE_LIT: {
            log(LOG_DEBUG, "ulang_type_lit\n");
            string_extend_ulang_type_lit(string, ulang_type_lit_const_unwrap(lang_type));
            log(LOG_DEBUG, "ulang_type_lit returning\n");
            return;
        }
    }
    unreachable("");
