...
#define poison name_new(MOD_PATH_ARRAYS, sv(""), (Ulang_type_darr) {0}, SCOPE_TOP_LEVEL)
...
Name serialize_ulang_type_fn(Strv mod_path, Ulang_type_fn ulang_type, bool include_scope) {
    ...
    return name_new(mod_path, string_to_strv(name), (Ulang_type_darr) {0}, SCOPE_TOP_LEVEL);
}
...
Name serialize_ulang_type_array(Strv mod_path, Ulang_type_array ulang_type, bool include_scope) {
    ...
    return name_new(MOD_PATH_ARRAYS, string_to_strv(name), (Ulang_type_darr) {0}, SCOPE_TOP_LEVEL);
}
...
static Name serialize_ulang_type_expr_lit(Strv mod_path, const Uast_expr* expr) {
    switch (expr->type) {
        ...
        case UAST_EXPR_REMOVED:
            ...
            return name_new(MOD_PATH_ARRAYS, sv(""), (Ulang_type_darr) {0}, SCOPE_TOP_LEVEL);
    }
    unreachable("");
}
