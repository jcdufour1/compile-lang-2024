...
#define poison name_new(MOD_PATH_ARRAYS, sv(""), (Ulang_type_darr) {0}, SCOPE_BUILTIN)
...
Name serialize_ulang_type_fn(Strv mod_path, Ulang_type_fn ulang_type, bool include_scope) {
    ...
    return name_new(mod_path, string_to_strv(name), (Ulang_type_darr) {0}, SCOPE_BUILTIN/*TODO*/);
}
...
Name serialize_ulang_type_array(Strv mod_path, Ulang_type_array ulang_type, bool include_scope) {
    ...
    return name_new(MOD_PATH_ARRAYS, string_to_strv(name), (Ulang_type_darr) {0}, SCOPE_BUILTIN);
}
...
static Name serialize_ulang_type_expr_lit_struct_literal(Strv mod_path, const Uast_struct_literal* lit) {
    ...
    darr_foreach(idx, Uast_expr*, memb, lit->members) {
        string_extend_strv(
            &a_main,
            &name,
    ...
    return name_new(MOD_PATH_ARRAYS, string_to_strv(name), (Ulang_type_darr) {0}, SCOPE_BUILTIN/*TODO*/);
}
...
static Name serialize_ulang_type_expr_lit(Strv mod_path, const Uast_expr* expr) {
    switch (expr->type) {
        ...
        case UAST_EXPR_REMOVED:
            ...
            return name_new(MOD_PATH_ARRAYS, sv(""), (Ulang_type_darr) {0}, SCOPE_BUILTIN);
    }
    unreachable("");
}
...
Name serialize_ulang_type_tuple(Strv mod_path, Ulang_type_tuple ulang_type, bool include_scope) {
    ...
    for (size_t idx = 0; idx < ulang_type.ulang_types.info.count; idx++) {
        Ulang_type curr = darr_at(ulang_type.ulang_types, idx);
        string_extend_strv(&a_main, &name, serialize_name(serialize_ulang_type(
            mod_path,
            curr,
            include_scope
    ...
    return name_new(mod_path, string_to_strv(name), (Ulang_type_darr) {0}, 0);
}
...
Name serialize_ulang_type_gen_param(Strv mod_path) {
    return name_new(mod_path, sv("gen_param"), (Ulang_type_darr) {0}, 0);
}

Name serialize_ulang_type_removed(Strv mod_path) {
    return name_new(mod_path, sv("removed"), (Ulang_type_darr) {0}, 0);
}

Strv serialize_ulang_type_darr(Strv mod_path, Ulang_type_darr darr, bool include_scope) {
    String name = {0};
